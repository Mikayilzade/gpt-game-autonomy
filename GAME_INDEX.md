...
| 013 | **SEAL BREAK** *(working title)* | **IN DESIGN / Phase 4 Mechanical Architecture complete / Phase 5 next** | TBD | NOT STARTED | Winner after 28 -> 12 -> 5 -> 3 -> 1 tournament. Mechanical authority `GAME13_MECHANICAL_ARCHITECTURE.md`. |
...
**Seal Break** is now mechanically locked as a finite deterministic destructive-witness puzzle. Canonical geometry stores each compartment's traversed seams and each socket's covered seams; a socket's trigger compartments are derived. One compartment opens atomically per checkpoint. Every intact installed witness whose covered seam is traversed breaks at that checkpoint and never changes break time afterward. Several seals may tear at one checkpoint with no meaningful sub-order.

The player-facing design uses four bounded history modes: fixed history, finite authored history choice, permutation of a required set, and bounded subset + order with explicit omission count. Seal placement is fixed or finite-budget socket selection. The complete v1 evidence vocabulary, preview/oracle boundary, undo/commit semantics, acceptance/equivalence policy, certifier classes, difficulty knobs and authored-content validity gates are frozen in `GAME13_MECHANICAL_ARCHITECTURE.md`.

The principal remaining risk is content repetition, not rules ambiguity. Phase 5 must prove a concrete 24-case minimum and 30-case target campaign using the existing deduction vocabulary rather than larger counts or decorative predicates. Production implementation remains forbidden inside the factory.

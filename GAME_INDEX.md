...
| 002 | **False Map Department** | **DESIGN COMPLETE / specification frozen** | Migration pending | Not applicable yet | Selected run 5; design frozen run 13 after Phase-11 32/32 implementation-readiness audit. Migration gate is next. |
...
Current state: **DESIGN COMPLETE = YES; safe migration to a dedicated repository is next.**

Technical direction on paper: **Godot 4.7.1-stable, GDScript-first, deterministic 2D domain separated from presentation.**

Final design authority while still in the factory: `GAME2_PHASE11_FINAL_FREEZE.md`.
...
## Migration rule
A game receives its own repository only once its design is sufficiently stable to justify migration, normally at `DESIGN COMPLETE = YES`. The dedicated repo owns implementation and all future game-specific work. The factory returns to a clean design slot afterward.

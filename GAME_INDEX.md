...
| 016 | **ONE-WAY WORKSHOP** *(working title)* | **IN DESIGN / Phase 8 technical specification complete / Phase 9 next** | TBD | NOT STARTED | Godot 4.7 direction; deterministic domain/presentation separation; exact schemas, atomic commands, validators, persistence, demo import, cloud merge, input/glyph, localization, budgets and future implementation order frozen in `GAME16_TECH_SPEC.md`. |
...
Game #016 Phases 1–8 are complete. **ONE-WAY WORKSHOP** remains selected. Active design authority now includes `GAME16_TECH_SPEC.md`; next step is Phase 9 Whole-Game Simulation on Paper. The canonical campaign remains exactly 24 authored cases across Immediate Byproduct, Property Choice, Delayed Lineage, Cross-Blank Ancestry, Dual-Use Conflict and Derived Witness Relay. Technical truth is finite/deterministic and independent of scene animation/physics; profile progression uses exact prerequisite evaluation and monotonic merge semantics. Rejected finalists Unpacking Order and Margin of Error remain tournament history only.

...
| 010 | **Luggage Carousel Zero** *(working title)* | **IN DESIGN / Phase 9 active** | TBD | NOT STARTED | Phases 1–8 complete; whole-game simulation active. |
...
- Phase 8: **complete**. `GAME10_TECHNICAL.md` + `GAME10_PHASE8_CLOSURE.md` freeze preferred Godot 4.7.2 + GDScript direction, pure deterministic Rules Core, canonical case/play schemas, exact solver/DEAD boundary, certificate/hash and content-manifest authority, atomic versioned persistence, demo/full merge, save migration, platform/input/localization/accessibility adapters, performance/test gates and implementation order.
- Phase 9: **active**. `GAME10_PHASE9_SIMULATION.md` has completed the first end-to-end paper walkthrough: first boot, demo/full teaching arc, first hour, early/mid/late mastery, K2, replay/Efficient Route, blocker skip, demo import, active-case resume, reinstall/cloud restore and hostile behavior. A second hostile closure pass remains.

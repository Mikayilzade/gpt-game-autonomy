...
| 014 | **Negative Casting** *(working title)* | **IN DESIGN / Phase 3 Product Thesis complete / Phase 4 next** | TBD | NOT STARTED | Round C selected Negative Casting over Missing Reflection and Casting Call after 24-case campaign/hour-3 stress tests. Product thesis locked. |
...
Game #014 Phase 1 generated the field in `GAME14_RESEARCH.md`; Round A evidence is in `GAME14_TOURNAMENT.md`; Round B evidence is in `GAME14_ROUND_B.md`; final product/campaign comparison and selection are in `GAME14_ROUND_C.md`; current product authority is `GAME14_PRODUCT_THESIS.md`.

**Selected Game #014 concept: Negative Casting.** Compact premium spatial-deduction puzzle: arrange socketed opaque blockers between two logical light channels so several projection surfaces match required LIT/L1_ONLY/L2_ONLY/BOTH states. Frozen depth sources are protected negative space, geometric endpoint/extent reasoning, channel attribution and cross-surface equivalence splitting. Projection masks must derive from coherent geometry, never arbitrary authored bitmasks. Missing Reflection and Casting Call are rejected tournament history and are not available as silent feature sources.

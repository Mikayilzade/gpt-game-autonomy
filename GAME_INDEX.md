...
| 016 | **ONE-WAY WORKSHOP** *(working title)* | **IN DESIGN / Phase 9 whole-game simulation complete / Phase 10 next** | TBD | NOT STARTED | End-to-end simulation passed first boot, demo/import, OW01–OW24 progression, hints, replay, Deck/controller, persistence/cloud/reset and hostile behavior; no unresolved structural contradiction. |
...
Game #016 Phases 1–9 are complete. **ONE-WAY WORKSHOP** remains selected. Active design authority now includes `GAME16_SIMULATION.md`; next step is Phase 10 Adversarial Review. The canonical campaign remains exactly 24 authored cases across Immediate Byproduct, Property Choice, Delayed Lineage, Cross-Blank Ancestry, Dual-Use Conflict and Derived Witness Relay. Phase-9 simulation verified the exact demo/import progression, state-based certification, late witness/occupation conflicts, controller-only OW24 traversal, save/crash boundaries and monotonic profile merge. Two technical clarity items are carried into adversarial review/final freeze: persist deliberate reset generation semantics, and freeze 1.0 active attempt as local-only while cloud-syncing the profile. Rejected finalists Unpacking Order and Margin of Error remain tournament history only.
